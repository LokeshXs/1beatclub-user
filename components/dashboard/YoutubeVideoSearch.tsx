...
export default function YoutubeVideoSearch() {
  ...
  const {setToggleSubscriptionModal} = useIsPremiumStore();
  const session = useSession();
  const [openYTSearchModal,setOpenYTSearchModal] = useState(false);
  ...
  const subscriptionCheckHandler = () =>{
    const isPremium = session.data?.user.isPremiumMember;

    if(isPremium){
      setOpenYTSearchModal(true);
    }else{
      setToggleSubscriptionModal(true);
    }
  }
  ...
  return (
      ...
      <Dialog
      open={openYTSearchModal}
        onOpenChange={() => {
          setSelectedVideo(undefined);
          setFetchedVideos([]);
          setOpenYTSearchModal(false);
        }}
      >
        <DialogTrigger className=" relative flex justify-center ">
          <ShimmerButton
            ...
            shimmerSize="0.2em"
            shimmerDuration="5s"
            onClick={subscriptionCheckHandler}
